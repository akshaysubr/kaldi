...
int main(int argc, char *argv[]) {
  try {
    ...
    std::vector<std::unique_ptr<SequentialTableReader<WaveHolder>>> wav_readers(
        iterations);
    for (int iter = 0; iter < iterations; ++iter)
      wav_readers[iter].reset(
...
#pragma omp parallel for
    for (int iter = 0; iter < iterations; iter++) {
      ...
      for (; !wav_readers[iter]->Done(); wav_readers[iter]->Next()) {
        ...
        std::string utt = wav_readers[iter]->Key();
        std::string key = utt;
        if (iter > 0) {
          // make key unique for subsequent iterations
          key = key + "-" + std::to_string(iter);
        }
        const WaveData &wave_data = wav_readers[iter]->Value();
        ...
        std::cout << "thread " << this_id << " sending...\n";

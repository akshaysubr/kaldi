...
static bool GetCudaContext(int32 num_gpus, std::string *debug_str) {
  ...
  for (int32 device = 0; device < num_gpus; device++) {
    ...
    if (e == cudaSuccess) {
      if (debug_str) *debug_str = debug_stream.str();
...
void CuDevice::Initialize() {
  ...
  if (device_id_ == -1) {
    ...
    if (!multi_threaded_) {
      ...
      KALDI_WARN
          << "For multi-threaded code that might use GPU, you should call "
    ...
    if (device_options_.use_tensor_cores) {
      ...
      // Note if the device does not support tensor cores this will fall back to
      // normal math mode
      CUBLAS_SAFE_CALL(
    ...
    CURAND_SAFE_CALL(
        curandCreateGenerator(&curand_handle_, CURAND_RNG_PSEUDO_DEFAULT));
    // To get same random sequence, call srand() before the constructor is
    // invoked,
...
void CuDevice::SelectGpuId(std::string use_gpu) {
  ...
  if (use_gpu != "yes" && use_gpu != "no" && use_gpu != "optional" &&
      use_gpu != "wait") {
    KALDI_ERR << "Please choose : --use-gpu=yes|no|optional|wait, passed '"
              << use_gpu << "'";
  }
  if (use_gpu == "no") {
  ...
  if (num_gpus == 0) {
    ...
    if (use_gpu == "optional") {
      KALDI_WARN << "No CUDA GPU detected; running on CPU since "
                    "--use-gpu=optional specified.";
      return;
    }
  }
...
bool CuDevice::SelectGpuIdAuto() {
  ...
  if (num_gpus == 0) {
    ...
    if (e != cudaSuccess) {
      KALDI_WARN << "cudaGetDeviceCount() returned " << e << ", meaning: \""
  ...
  KALDI_LOG << "Success selecting device " << dev_id
            << " free mem ratio: " << mem_ratio;
  return true;
}

void CuDevice::AccuProfile(const char *function_name, const CuTimer &timer) {
...
void CuDevice::PrintMemoryUsage() const {
  if (Enabled()) g_cuda_allocator.PrintMemoryUsage();
}
...
CuDevice::~CuDevice() {
  if (cublas_handle_) CUBLAS_SAFE_CALL(cublasDestroy(cublas_handle_));
  if (cusparse_handle_) CUSPARSE_SAFE_CALL(cusparseDestroy(cusparse_handle_));
  if (curand_handle_) {

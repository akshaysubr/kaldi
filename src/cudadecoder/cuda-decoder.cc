...
void CudaDecoder::InitDecoding(const std::vector<ChannelId> &channels) {
  ...
  for (ChannelId ichannel : channels) {
    ...
    // TODO put it back
    // if (thread_pool_) {
    //  thread_pool_->post([ichannel, this] { InitDecodingH2HCopies(ichannel);
    //  });
    //} else
...
void CudaDecoder::CheckOverflow() {
  for (LaneId ilane = 0; ilane < nlanes_used_; ++ilane) {
    ...
    if (q_overflow != OVERFLOW_NONE) {
      ...
      // The algorithm can still go on but quality of the result can be
      // reduced
      ...
      if ((q_overflow & OVERFLOW_MAIN_Q) == OVERFLOW_MAIN_Q) {
        ...
        KALDI_WARN
            ...
            << "To prevent this from happening, please increase the "
               "parameter "
               "--main-q-capacity"
...
void CudaDecoder::AddFinalTokensToLattice(
  ...
  for (auto &p : h_all_final_tokens_list_[ichannel]) {
    ...
    // We need to create the fst_lattice_state linked to our internal id in
    // the
...
void CudaDecoder::AddArcToLattice(
  ...
  // And propagate the extra cost from the destination to the source of that
  // arc
  // (we go backward)
  ...
  if (list_prev_token_idx != 0) {
    ...
    // Reading the OutputLatticeState of the source state in the output
    // lattice
...
void CudaDecoder::ConcurrentGetRawLatticeSingleChannel(const ChannelId ichannel,
  ...
  // Freeing some memory. Necessary when using the decoder as a service
  if (h_all_tokens_info_[ichannel].capacity() >
      2 * ntokens_pre_allocated_) {  // TODO
...
void CudaDecoder::ComputeH2HCopies() {
  ...
  // If we're the last cpu thread to complete the current tasks, notify the
  // main
  // thread
  bool all_done;
  {
